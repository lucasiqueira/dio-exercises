...
public class SistemaAcionistas {
    ...
    public List<String> obterAnalisesDesempenho(String dataInicialStr, String dataFinalStr)
        ...
        List<String> filtrada = new ArrayList<String>();
        for (Analise analise : analises) {
            if (analise.data.compareTo(dataInicial) >= 0
                && analise.data.compareTo(dataFinal) <= 0) {
                filtrada.add(analise.descricao);
            }
        }
        return filtrada;
